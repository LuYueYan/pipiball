...
class runningScene extends eui.Component implements eui.UIComponent {
	...
	public adaptParams = {
		gridAreaTop: 319 + 9,//格子区域距离屏幕顶部距离
	...
	public myData = {
		...
		bulletIndex: userDataMaster.bulletIndex//本局的炮弹类型索引
	};
	...
	public shootPoint = { bx: 375, by: 1154, ex: 0, ey: 0, floor: false, beeNum: 0, speedy: 3, shootAll: false };
	...
	public constructor(level, tool = { glass: false, bullet: false }) {
		super();
		// console.log(level)
	...
	public init() {
		...
		if (userDataMaster.tryingIndex >= 0) {
			that.myData.bulletIndex = userDataMaster.tryingIndex;
			userDataMaster.tryingIndex = -1;
		}
		that.myData.beeNum = that.levelInfo.bullet;
		that.levelText.text = '第' + that.levelInfo.level + '关';
		that.bulletImg.texture = RES.getRes(userDataMaster.bulletArr[that.myData.bulletIndex].img + '_png');
			...
			setTimeout(function () {
				...
				gif.addEventListener(egret.Event.COMPLETE, (e: egret.Event) => {
					for (let i = 0, len = 5; i < len; i++) {
						setTimeout(function () {
							let bee = new beeCom(that.myData.bulletIndex);
		...
		that.toolState(t);
		that.homeBtn.addEventListener(egret.TouchEvent.TOUCH_TAP, that.homeFun, that);
		...
		let recommand = runningCommand.getInstance();
		that.recommandGroup.addChild(recommand);
		egret.Tween.removeTweens(recommand.group_1);
		recommand.group_1.rotation=0;
		egret.Tween.removeTweens(recommand.group_2);
		recommand.group_2.rotation=0;
		egret.Tween.get(recommand.group_1, { loop: true }).wait(2000).to({ rotation: -15 }, 300).to({ rotation: 15 }, 600).to({ rotation: 0 }, 300);
		egret.Tween.get(recommand.group_2, { loop: true }).wait(3000).to({ rotation: -15 }, 300).to({ rotation: 15 }, 600).to({ rotation: 0 }, 300);

		CallbackMaster.playCallback = function () {
			console.log('onshow')
	...
	public dataChange(e: eui.CollectionEvent) {
		...
		for (let i = 0; i < tool.length; i++) {
			...
			if (userDataMaster.level + 1 >= item.level) {
				//已解锁
				if (userDataMaster.tool[tool[i]].num > 0) {
					that[tool[i] + '_num'].text = userDataMaster.tool[tool[i]].num + '';
					that[tool[i] + '_numrect'].fillColor = 0x64C101;
				} else {
					that[tool[i] + '_num'].text = '+';
					that[tool[i] + '_numrect'].fillColor = 0xF7585C;
				}
			}
		}
	...
	public toolState(t) {
		...
		for (let i = 0; i < tool.length; i++) {
			...
			if (userDataMaster.level + 1 >= item.level) {
				...
				that[tool[i] + '_num'].parent && that[tool[i]].removeChild(that[tool[i] + '_num']);
				that[tool[i] + '_numrect'].parent && that[tool[i]].removeChild(that[tool[i] + '_numrect']);
	...
	public judgeTool(type, i) {
		...
		that.toolModal = new useToolModal(type);
		sceneMaster.openModal(that.toolModal);
		that.toolModal.useBtn.addEventListener(egret.TouchEvent.TOUCH_TAP, that.useToolFun, that);
		...
		// 		that[type + '_numrect'].fillColor = 0x64C101;
		// 	} else {
		// 		that[type + '_num'].text = '+';
		// 		that[type + '_numrect'].fillColor = 0xF7585C;
		// 	}
		// 	if (type == 'hammer') {
		// 		//锤子
	...
	public useToolFun() {
		let that = this;
		let toolModal = that.toolModal;
		if (userDataMaster.tool[toolModal.type].num > 0) {
			userDataMaster.tool[toolModal.type].num--;
			userDataMaster.myTool = userDataMaster.tool;
			that.usingToolFun(toolModal.type);
		} else if (userDataMaster.gold >= toolModal.arr[toolModal.type].price) {
			userDataMaster.myGold = userDataMaster.gold - toolModal.arr[toolModal.type].price;
			that.usingToolFun(toolModal.type);
		} else {
			AdMaster.useVideo(()=>{
				that.usingToolFun(toolModal.type);
	...
	public updateProccess(conTimes) {
		...
		if (conTimes.num > 2) {
			...
			if (Math.random() < 0.7 || that.myData.existAmount >= that.levelInfo.amount) {
				...
				sceneMaster.openModal(new continueGift());
			}
		}
		that.scoreText.text = that.myData.score + '';
		that.changeGraphics();
	...
	public createGrids(row = 1, first = false) {
		...
		for (let col = 0; col < 7; col++) {
			...
			if (ran < 0.45 && (that.myData.existAmount < that.levelInfo.amount)) {
				...
				g = new ballCom(that.myData.bulletIndex);
			} else if (ran < 0.62) {
				g = new starCom();
			} else {
				continue;
	...
	private onEnterFrame() {
		...
		for (let len = that.gridArr.length, i = len - 1; i >= 0; i--) {
			if (that.gridArr[i].type == 6 && Math.abs(that.gridArr[i].boxBody.velocity[0]) > 0) {
				...
				let x = 6 * that.adaptParams.itemWidth + that.adaptParams.itemWidth / 2;
				if (that.gridArr[i].boxBody.position[0] < that.getPosition(that.adaptParams.itemWidth / 2, 2) && k < 0) {
					k = -k;
				}
				if (that.gridArr[i].boxBody.position[0] > that.getPosition(x, 2) && k > 0) {
					k = -k;
				}
	...
	public changeGraphics() {
		...
		let w = 0;
		if (that.myData.score <= that.levelInfo.score) {
			w = that.myData.score / that.levelInfo.score * (that.star_1.x - that.proccessBar.x + 10);
		} else if (that.myData.score <= that.levelInfo.score * 3) {
			w = that.star_1.x - that.proccessBar.x + 10 + (that.myData.score - that.levelInfo.score) / (that.levelInfo.score * 2) * (that.star_2.x - that.star_1.x);
		} else {
			w = that.star_2.x - that.proccessBar.x + 10 + (that.myData.score - that.levelInfo.score * 3) / (that.levelInfo.score * 3) * (that.star_3.x - that.star_2.x);
		}
		that.proccessBar.width = w>360?360:w;
